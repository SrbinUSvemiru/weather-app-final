...
function VisibilitySvg({ clicked, currentCity, activeWrapper }) {
	...
	useEffect(() => {
		switch (clicked) {
			case 'hourly':
				setVisibility(() => {
					let values = data.hourly.map((element, index) => element.humidity / 2);
	...
	const animation = useSpring({
		from: { opacity: 0 },
		to: {
			opacity: activeWrapper === 'visibility' ? 1 : 0,
		},
	});
